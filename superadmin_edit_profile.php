...
if (isset($_POST['submit'])) {
    ...
    if ($new_password != "") {
        if (!validatePasswordLength($new_password)) {
            ...
            header("Location: superadmin_profile.php?error=Password must be at least 8 characters long");
            exit();
        }
        ...
        // Update superadmin's profile in the database with password
        ...
        // Update superadmin's profile in the database without password
        $update_query = "UPDATE user SET username=?, email=? WHERE id=?";
        $update_stmt = $connect->prepare($update_query);
        $update_stmt->bind_param("ssi", $new_username, $new_email, $superadmin_userid);
    }
